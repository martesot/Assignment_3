...
def simulate_compound_returns(starting_x, starting_s2, theta, use_leverage: bool=False, T: int=20) -> np.ndarray:
    ...
    for t in range(T):
        s2 = GARCH_s2_formula(s2, x, theta, use_leverage=use_leverage)
        x = s2 * np.random.standard_t(df=theta[3])

        compound_return_factor *= (1 + x / 100)
        compound_returns[t] = 100 * (compound_return_factor - 1)
        
    return compound_returns
        
    
def question_6(data_dict: dict, nolev_dict: dict, lev_dict: dict, T: int=2500) -> None:
    """Answers question 6: """
    levels = [0.01, 0.05, 0.1]
    for stock_name, stock_data in data_dict.items():        
        ...
        print(f"\nNo leverage")
        s2 = np.var(stock_data.RET.iloc[0:50]) # This one doesn't really matter
        for t in range(T-1):
            s2 = GARCH_s2_formula(s2, stock_data.RET.iloc[t], nolev_dict[stock_name])
        
        for level in levels:
            backtesting_procedure(stock_data.iloc[T-1:], nolev_dict[stock_name], s2, level)
        ...
        for level in levels:
            backtesting_procedure(stock_data.iloc[T-1:], lev_dict[stock_name], s2, level, use_leverage=True)
            

def backtesting_procedure(stock_data: pd.DataFrame, params: np.ndarray, starting_s2: float, level: float, T: int=2500, use_leverage: bool=False):
    """Backtests the GARCH model."""
    H = len(stock_data) - 1
    y = np.zeros(H)
    s2 = starting_s2
    for t in range(H):
        s2 = GARCH_s2_formula(s2, stock_data.RET.iloc[t], params, use_leverage=use_leverage)
        t_quant = sts.t.ppf(level, params[3])
        if (t_quant * np.sqrt(s2)) > stock_data.RET.iloc[t+1]: y[t] = 1
...
def main() -> None:
    ...
    nolev_dict = { # From the examples
        'KO':  np.array([0.005665683, 0.09922153, 0.91055318, 5.5415955]), 
        'PFE': np.array([0.02897666, 0.11873272, 0.88174828, 6.17490427]), 
        'JNJ': np.array([0.01349871, 0.15553047, 0.85263037, 5.95413620]), 
        'MRK': np.array([0.04942468, 0.16251609, 0.83947497, 4.38238453])
    }
